...
##*
##*  Team ID : 3925
##*  Team Leader Name: Pahul Singh Sawhney
##*  Team Member Name : Mohit Kumar Gautam
##*
##*  
...
function dy = RW_pendulum_dynamics(y, m1, m2, l1, wr, g, u)
  ...
  I1 = (m1*l1^2)/12;
  I2 = (m2*wr^2)/2;
  a = (I1 + m1*(l1/2)^2) + m2*l1^2;
  b = (m1/2 + m2)*l1; 
  dy(1,1) = y(2);         
  dy(2,1) = b*g/a*sin(y(1)) - u/a;
  dy(3,1) = -b*g/a*sin(y(1)) + (1/a + 1/I2)*u;
endfunction
...
function [A, B] = RW_pendulum_AB_matrix(m1 , m2, l1, wr, g)
  I1 = m1*(l1^2+0.05^2)/12;          # Moment of Inertia of pendulum bar
  I2 = (m2*wr^2)/2;            # Moment of Inertia of reaction wheel 
  a = (I1 + m1*(l1/2)^2) + m2*l1^2;
  b = (m1/2 + m2)*l1*g; 
  A = [0 1 0; b/a 0 0; -b/a 0 0];
  B = [0; -1/a; (1/a + 1/I2)]; 
...
function [t,y] = lqr_RW_pendulum(m1, m2, l1, wr, g, y_setpoint, y0) 
 [A, B] = RW_pendulum_AB_matrix(m1 , m2, l1, wr, g);
  Q = [2 0 0 ; 0 0.02 0; 0 0 0.001];
  R = 1;                             ## Initialise R 
  K = lqr(A,B,Q,R); 
  # K=[-101.2354436 -28.1198273 -0.0011140 -0.0173285]
...
function RW_pendulum_main()
  ...
  y0 = [209*pi/180; 0; 0]; # Initial Conditions
  y_setpoint = [pi; 0; 0];  # Reference Point

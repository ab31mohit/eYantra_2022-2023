...
function jacobian_matrices = find_jacobian_matrices(x_1, x_2, x1_dot, x2_dot)
  ...
  for i=1:size(solutions,1)
     jacobian_matrices{i}=double(subs(j,v,[solutions(i,1) solutions(i,2)]));
  endfor
  ################## ADD YOUR CODE HERE ######################
...
function [eigen_values stability] = check_eigen_values(x_1, x_2, jacobian_matrices)
  ...
  for k = 1:length(jacobian_matrices)
    ...
    eig_value_matrix=eigen_values{k};
    for i=1:length(eig_value_matrix)
      if(real(eig_value_matrix(i)) > 0)
        flag=0;
      endif
    endfor
